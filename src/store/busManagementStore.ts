...
import { TamilNaduCity } from '../data/ci';

interface BusManagementState {
  buses: Bus[];
  isLoading: boolean;
  error: string | null;
...
export const useBusManagementStore = create<BusManagementState>((set, get) => ({
  buses: [],
  isLoading: false,
  error: null,
  ...
  addBus: async (busData) => {
    ...
    try {
      ...
      set({ error: 'Failed to add bus', isLoading: false });
    }
  },
    ...
    try {
      ...
      set(state => ({
        buses: state.buses.map(bus => 
          bus.id === id ? { ...bus, ...updates } : bus
        ),
        isLoading: false,
      }));
    } catch (error) {
      set({ error: 'Failed to update bus', isLoading: false });
    }
  },
  ...
  deleteBus: async (id) => {
    ...
    try {
      ...
      set({ error: 'Failed to delete bus', isLoading: false });
    }
  },
}));